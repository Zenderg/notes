...
Работает как [`mergeScan`](#mergescan), но отправляет в подписку только последний отработанный обзервабл. Т.е. если в `switchScan` попадает куча ивентов, до того как предыдущий обзервабл завершился, то он обработает только последний пришедший ивент. Обзерваблы, которые должны были вернутся на предыдущие ивенты, он отменит.

Да и вообще походу все операторы с приставкой `switch` работают всегда только с последними ивентами и отменяют предыдущие.

> https://rxjs.dev/api/operators/switchScan

## switchMap

Возвращает обзервабл, значение которого пойдет дальше по пайпу. 

* Если из возвращаемого обзервабла будут прилетать ивенты, то они также будут попадать в обработку и проходить по пайпу.
* Если из основного обзервабла прилетает новый ивент, то `switchMap` остановит обработку обзервабла предыдущего ивента и будет следить уже за новым.
...
Задеприкейчено. Юзайте [`switchMap`](#switchmap) и будет вам счастье.
